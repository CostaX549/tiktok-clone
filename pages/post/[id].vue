<template>
    ...
    />
     ...
     <div id="InfoSection"  v-if="$generalStore.selectedPost" class="lg:max-w-[550px] relative w-full h-full bg-white">
      <div class="py-7" />
      ...
      <div class="flex items-center px-8 mt-8">
          <div class="pb-4 text-center flex items-center">
             ...
             <span class="text-xs pl-2 pr-4 text-gray-800 font-semibold">{{  $generalStore.selectedPost.likes.length }}</span>
          </div>
          <div class="pb-4 text-center flex items-center">
...
try {
        ...
        if (error && error.response.status === 400) {
            router.push('/')
              ...
              setTimeout(() => {
                isLoaded.value = true;
                video.value.src = $generalStore.selectedPost.video;  
                video.value.play() 
              }, 1000);
